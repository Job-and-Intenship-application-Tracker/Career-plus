...
export default function FollowUpNotesView({ jobs = [], onSelectJob }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-xs md:text-sm text-amber-100 leading-relaxed font-medium">
              Below are all job applications with no status updates for more than 7 days. These cards are highlighted in a distinct yellow theme to grab your attention.
            </p>
          </div>
      ...
      {/* Grid of All Follow-Up Application Cards in Distinct Yellow Theme */}
        ...
        {followUpJobs.map((job) => {
          ...
          return (
            <div
              ...
              className="bg-gradient-to-br from-amber-50 via-amber-100/70 to-orange-50 rounded-3xl p-6 border-2 border-amber-400 shadow-md shadow-amber-500/10 hover:shadow-2xl hover:border-amber-500 transition-all duration-300 relative space-y-4 cursor-pointer group"
            >
              {/* Card Header & Avatar */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center space-x-3.5">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 text-white font-black text-lg flex items-center justify-center shadow-md shadow-amber-600/20 group-hover:scale-105 transition-transform">
                    {logo}
                  </div>
                  <div>
                    <h3 className="text-base font-extrabold text-slate-900 group-hover:text-amber-800 transition-colors line-clamp-1">
                      {title}
                    </h3>
                    <p className="text-xs font-bold text-slate-600 mt-0.5 flex items-center">
                      <span>{company}</span>
                      <span className="ml-2 px-2 py-0.5 bg-amber-200/80 text-amber-900 rounded-md text-[10px]">
                        {job.workMode || 'Remote'}
                      </span>
                    </p>
                  </div>
                ...
                <div className="flex-shrink-0">
                  <span className="bg-amber-200 text-amber-950 border border-amber-300 px-3 py-1 rounded-full text-xs font-black flex items-center space-x-1 shadow-2xs animate-pulse">
                    <AlertTriangle className="w-3.5 h-3.5 text-amber-800 mr-1" />
              ...
              <div className="grid grid-cols-2 gap-3 text-xs pt-1">
                ...
                {/* Offered Salary */}
                <div className="bg-emerald-50/90 border border-emerald-200/90 p-2.5 rounded-2xl space-y-0.5">
                ...
                {/* Recruiter Info */}
                <div className="bg-amber-100/80 border border-amber-200 p-2.5 rounded-2xl space-y-0.5">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-amber-900">Recruiter Contact</span>
                ...
                {/* Location */}
                <div className="bg-white/80 border border-amber-200/80 p-2.5 rounded-2xl space-y-0.5">
              ...
              <div className="pt-3 border-t border-amber-200/80 flex items-center justify-between gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white rounded-xl text-xs font-black flex items-center justify-center space-x-2 transition-colors shadow-md shadow-amber-600/20 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-4 bg-white hover:bg-amber-100 text-slate-800 rounded-xl text-xs font-bold flex items-center justify-center space-x-1.5 transition-colors cursor-pointer border border-amber-300"
                >
